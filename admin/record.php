...
<body>
	...
	<div class="container-fluid" style="margin-top:70px;">
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content ">
					<div class="modal-body">
						...
						<br />
						<center><a class="btn btn-danger remove_id"><span class="glyphicon glyphicon-trash"></span>
		...
		<?php
		$timeRes = "select * from time";
		$timeresult = mysqli_query($conn, $timeRes);

		if (isset($_POST["export"])) {
			$filename = "library_attendance.xls";
		...
		<form action="" method="post">
			<button class=" btn-success btn btn-sm" name='export' style="margin-bottom: 10px;"
				onclick="ExportToExcel('xlsx')">Export<i class="fa-solid fa-print" style="margin-left: 5px"></i>
			</button>
		</form>
		...
		<div class="well col-lg-12">
			...
			<table id="table1" class="table table-bordered">
				...
				<tbody>
					<?php
					$q_time = $conn->query("SELECT * FROM `time` ") or die(mysqli_error($conn));
					while ($f_time = $q_time->fetch_array()) {
						?>
						<tr>
							<td>
								<?php echo $f_time['student_no'] ?>
							</td>
							<td>
							...
							<td>
								<?php echo date("m-d-Y", strtotime($f_time['date'])) ?>
							</td>
							<td><button name="<?php echo $f_time['time_id'] ?>" class="btn btn-danger rtime_id" href="#"
