...
<html lang = "eng">
	...
	<body>
		...
		<div class = "container-fluid" style = "margin-top:70px;">
			...
			<br />
			<div class = "alert alert-success">Faculty Entered</div>
				...
				<div class = "modal-dialog" role = "document">
					<div class = "modal-content ">
						<div class = "modal-body">
							...
							<br />
							<center><a class = "btn btn-danger remove_id1" ><span class = "glyphicon glyphicon-trash"></span> Yes</a> <button type = "button" class = "btn btn-warning" data-dismiss = "modal" aria-label = "No"><span class = "glyphicon glyphicon-remove"></span> No</button></center>
						</div>
					</div>
				</div>
			</div>
			<div class = "well col-lg-12">
				<table id = "table1" class = "table table-bordered">
					...
					<tbody>
						<?php
							$q_timef = $conn->query("SELECT * FROM `timef` ") or die(mysqli_error());
							while($f_timef = $q_timef->fetch_array()){
						?>
						<tr>
							<td><?php echo $f_timef['faculty_no']?></td>
							<td><?php echo ($f_timef['faculty_name'])?></td>
							<td><?php echo date("h:i a", strtotime($f_timef['time']))?></td>
							<td><?php echo date("m-d-Y", strtotime($f_timef['date']))?></td>
							<td><button name = "<?php echo $f_timef['timef_id']?>" class = "btn btn-danger rtimef_id" href = "#" data-toggle = "modal" data-target = "#delete"><span class = "glyphicon glyphicon-remove"></span></button></td>
						</tr>
					<?php
						}
					?>
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			$('.rtimef_id').click(function(){
				$timef_id = $(this).attr('name');
				$('.remove_id1').click(function(){
					window.location = 'delete_timef.php?timef_id=' + $timef_id;
				});
			});
		});
	</script>
