...
<body class="alert-primary">
	<nav class="navbar bg-success navbar-fixed-top ">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav navbar-right">
				<li>
				<li><a class="text-success" href="index1.php"><i class="glyphicon glyphicon-user"></i> Faculty Login</a>
				</li>
				</li>
				<li>
				<li>
					<a class="text-success" href="admin/index.php"><i class="glyphicon glyphicon-user"></i> Admin
						Login</a>
				</li>
			</ul>
		</div>
				...
				<form method="POST" action="save.php" enctype="multipart/form-data">
					<div class="modal-body">
						<div class="form-group">
							...
							<input type="password" id="idnumber" name="student_no" required="required"
								class="form-control" />
							...
							<div class="col-md-12 input-group mb-4">
								...
								if (mysqli_num_rows($course_result) > 0) {
									...
									<select required name="course" class="form-control">
										...
										<?php
										while ($courseItem = mysqli_fetch_assoc($course_result)) {
											?>
											<option value="<?php echo $courseItem['course']; ?>">
												<?php echo $courseItem['course']; ?>
											</option>
											<?php
										}
										?>
					...
					<div class="modal-footer">
						<button class="btn btn-success" name="save"><span class="glyphicon glyphicon-save"></span>
							Save</button>
					</div>
				</form>
			</div>
	...
	<div class="container-fluid">
		...
		<div class="col-lg-4 well">
			...
			<br />
			<h2 class="text-success">Students Login</h2>
			...
			<div id="result"></div>
			<label class="text-success text-center">Student ID:</label>
			<input type="password" id="student" class="form-control" required="required" />
			<div id="error"></div>
			<br />
			<button type="button" id="login" class="btn btn-success btn-block"><span
...
<div class="col-lg-8 well">
	<table id="table1" class="table table-bordered">
		...
		<tbody>
			...
			$currentMonth = date("m");
			$currentYear = date("Y");
			$q_time = $conn->query("SELECT * FROM `time` WHERE MONTH(date) = $currentMonth AND YEAR(date) = $currentYear") or die(mysqli_error($conn));
			while ($f_time = $q_time->fetch_array()) {
				?>
				<tr>
					<td>
						<?php echo ($f_time['student_name']) ?>
					</td>
					<td>
						<?php echo date("h:i a", strtotime($f_time['time'])) ?>
					</td>
					<td>
						<?php
						$setdate = strtotime($f_time['date']);
